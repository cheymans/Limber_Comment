...
amplitude that is well within the Monte-Carlo sampling noise. \mk{The largest
difference is measured with the depreciated case ExtL1Fl, for which the
recovered amplitude is larger by $16\%$ of the statistical error.} \mk{These
negligible changes relative to the error bars were to be expected owing to the
much larger cosmic variance, compared to the differences in the various
approximations shown in Fig.~\ref{fig:Cl_cases}, which has been taken into
account in the calculation of the uncertainty.}
...
\mk{We also find that the cosmological constraints from tomographic KiDS are robust
to the flat-sky and Limber approximations. The case ExtL1FlHyb that was used
for KiDS data in \citep{KiDS-450} and \cite{joudaki/etal:2017} introduces
errors that are more than an order of magnitude lower than the cosmic variance
for that survey, and thus this approximation has negligible effect on
