...
correlation function is accurate well below one percent up to a few hundred arc
minutes. Using the fast Hankel transformation, formally defined only in flat
...
\citep{FSHK08}, \mk{and lead to spurious B-modes \cite{asgari/etal:2017}}. Here
...
for the spherical ones.} \mk{For $\xi_-$ which is much less dominated by large
scales and thus Limber and flat-sky approximations than $\xi_+$, we use the
Hankel transofm in all cases. In addition, for $\xi_-$ the approximation
...
\begin{figure}
  ...
  transform (\ref{eq:xi_Legendre}). For the flat cases, and all instances of $\xi_-$, the Hankel transform
  (\ref{eqn:xiGG}) was used.}
  The upper panels
