...
models meet the accuracy of the observations. In this paper, we examine the widely used Limber
...
projection. We show
that the second-order Limber approximation is an accurate representation of the full projection, with better than percent level precision for scales $\ell > 3$. 
Since this approximation involves only 1D integrals over the matter power spectrum, it is very fast to
calculate numerically and can be readily employed in Monte-Carlo sampling methods
...
survey areas with an extent less than 10 degrees. The correlation
...
\end{enumerate}
%
\ch{Would this section be clearer if we had $p(\ell)$ in the equations instead of 
$\ell^4/\nu^4$?}
