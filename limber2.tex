...
numerically and can be readily employed in Monte-Carlo sampling methods to
...
approximation. For completeness we provide a derivation of the weak lensing power spectra in Appendix~\ref{sec:derivations_C}.
...
S_K(|K|^{1/2} \chi)$ is the the comoving angular diameter distance, where the function
...
Accounting for this position-dependence leads to corrections of weak-lensing
...
spectrum is $p(\ell) := \ellbar^2(\ell, 2)$, which will be modified under
...
first approximate the matter power cross-spectrum between two
distances by the
geometric mean of the two involved distances \citep{2005PhRvD..72b3516C,2016arXiv161200770K},
%
\begin{equation}
...
\begin{align}
  ...
  P_{\rm m}\left(\frac{\nu}{\chi}; \chi\right) . \Label{ExtL1Sph}
...
non-linear scale $k_{\rm NL}(z)$ for \texttt{halofit} \citep{2003MNRAS.341.1311S}.

With this approximation, the \mk{$\chi$-independent term $P_{\rm m}(k)$}
factors out from equation~(\ref{eq:f_LA08}), and we define the separated kernel
function $f_{\rm s}$ as
%
\begin{equation}
...
\citep{CFHTLenS-2pt-notomo} and we assume their best-fit flat $\Lambda$CDM cosmology with $\Omega_{\rm m}=0.279$, $\Omega_{\rm b}=0.046$, $\sigma_8=0.79$, $H_0=0.701$, $n_{\rm s}=0.96$. 
...
\begin{centering}
  ...
  $2^{\rm st}$-order extended Limber, spherical & ExtL2Sph & (\ref{eq:C_ell_limber1})+(\ref{eq:C_ell_limber2_dr_s})
    & $\frac{\ellbar^2(\ell, 2)}{(\ell+1/2)^4}$ & $\ell+\frac 1 2$ & Best approximation but mixing with flat-sky Hankel transform \\ \hline
  %
  Full spherical & FullSph & (\ref{eq:C_ell_full}) &
      $\ellbar^2(\ell, 2)$ & - & Correct projection \\ \hline
  %
  \end{tabular}
...
On a sphere, correlation functions cannot be related to the power spectrum by
the Hankel transform in equation~(\ref{eqn:xiGG}), as the spherical power
spectrum is formally not defined for non-integer $\ell$ \citep[see][for
alternative spherical-sky formulae for the two-point correlation
function]{2005PhRvD..72b3516C}. In particular, the spherical Bessel functions
necessary to compute the full projection in equation~(\ref{eq:C_ell_full}) is
defined only for integer order $\ell$. \mk{I think this is actually not true,
since J$_\nu$ can be computed for fractional $\nu$, and then one could use the
identity (\ref{eq:jJ}). Or is it?}.
