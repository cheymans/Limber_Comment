...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Results}
\subsection{Comparison of the approximations}
\label{sec:comp}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
the range of different approximations listed in Table~\ref{tab:Cl_cases}.  The adopted redshift distribution corresponds to CFHTLenS
\cite{CFHTLenS-2pt-notomo} and we assume a flat $\Lambda$CDM cosmology with $\Omega=X$, $\Omega_b=$, $\sigma_8=X$, $H_0=X$, $n_s=X$. 
...
accurate to better than 10\% for $\ell>3$, converging slowly to the true projection with percent level predicison at $\ell>100$. 
...
reach 10\% precision only at $\ell > 100$.   To our knowledge this form of the flat-sky approximation has not been used in cosmic shear studies to date.
...
\begin{figure}
  ...
  \caption{Shear power spectrum for different approximation. Limber to first order: standard with flat-sky (L1Fl),
       ...
       \ch{ExtL2Fl isn't in the table - so suggest to remove that from both plots}
        }
...
\begin{table}
  ...
  %
  $2^{\rm st}$-order extended Limber, flat sky & ExtL2FlHyb &  (\ref{eq:P_ell_limber1}) + (\ref{eq:P_ell_limber2}) + (iii)
    & \ch{I think this should be 1?}%$\ellbar^2(\ell, 2)/(\ell+1/2)^4$ 
    & $\ell+1/2$ & - \\ \hline
  %
