...
\begin{enumerate}
  ...
  i.e.~for all pre-2014 CFHTLenS results, DES-SV \citep{abbott/etal:2016} and DLS
...
approximation, (L1Fl), that was adopted for all pre-2014 CFHTLenS analyses, DES-SV and DLS analyses, we
...
\begin{centering}
  ...
  $1^{\rm st}$-order standard Limber, flat sky & L1Fl & (\ref{eq:P_ell_limber1}) + (i)
    & $1$ & $\ell$ & Pre-2014 CFHTLenS, DES-SV and DLS \\ \hline
  %
  ...
  $1^{\rm st}$-order extended Limber, hybrid, flat sky & ExtL1FlHyb & (\ref{eq:P_ell_limber1}) + (iii)
    & $1$ & $\ell + \frac 1 2$ & Post-2014 CFHTLenS, KiDS \\ \hline
  %
...
be replaced by the spherical transform \citep{1973ApJ...185..413P}
%
\begin{equation}
 \xi_+(\theta) = \left\langle \gamma \gamma^\ast \right\rangle(\theta)
  =  \frac 1{4\pi} \sum\limits_{\ell=2}^\infty (2 \ell + 1) C^\gamma(\ell) {\rm P}_\ell[\cos(\theta)],
  \label{eq:xi_Legendre}
\end{equation}
%
where ${\rm P}_\ell(x)$ are the Legendre polynomials in $x$ of order $\ell$. This equation is easily derived
...
spherical transform (\ref{eq:xi_Legendre}) is well below one percent for
$\vartheta < 200$ arcminutes. The red circles in Fig.~\ref{fig:xi_p_legendre}
present the limit of precision that we can achieve with the current fast Hankel
transform implementation for the correlation function. The green line shows the
limit of the second-order Limber approximation on the correlation function.
