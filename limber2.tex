...
\newcommand{\Label}[1]{\quad \mk{(\mbox{#1})}}
\newcommand{\LabelTxt}[1]{\mk{(\mbox{#1})}}
...
contrast to the recent claim made by \citet{2016arXiv161104954K}. For future
...
\mk{and the corresponding errors an order of magnitude below cosmic
variance for all $\ell$.}
%
...
the full projection. \mk{The first-order extended Limber approximation provides
...
We \mk{further} show that the second-order Limber approximation is an
...
from \citet{2000PhRvD..62d3007H}, see also \citet{2005PhRvD..72b3516C}. In
...
compare the shear power spectrum and commonly-used two-point shear correlation
function for the full solution to a range of different approximations in
section~\ref{sec:results}, providing cosmological constraints for each case
using CFHTLenS data from \citet{CFHTLenS-2pt-notomo}. This paper draws from
several sources of literature which have previously discussed the full
...
The lensing potential $\psi$ at a direction (\MA {position}?) on the sky $(\theta, \varphi)$ in
the Born approximation is defined as the projected 3D metric potential $\Phi$
along the line of sight \mk{of a flat Universe} \citep{1998ApJ...498...26K,BS01},
%
...
redshift distribution $n_z(z) {\rm d}z = n(\chi) {\rm d} \chi$, \MA{with  $\chi_{\rm h}$,
the comoving distance to the horizon.} Here, ${\rm c}$ is the speed
of light, and the projection is carried out over comoving distances $\chi$.
...
\mk{The last argument of the potential $\Phi$ is not to
...
constant, $\vec k$ a 3D Fourier wave vector, \mk{with $|\vec k|$ being the comoving
...
to the \MA{cosmic microwave background (CMB)}
...
\end{align}
%
\mk{Complex conjugation is denoted with the superscript $^\ast$.}
...
\mk{Peter's comment: What does the correlation between two
epochs actually mean if they are not on the backward light cone?}
...
\end{align}
%
\mk{for $0 \le s \le \ell$,} with the spin pre-factor \citep{2012PhRvD..86b3001B}
%
\begin{equation}
...
The spherical \mk{spin} pre-factor for the full projection \mk{shear} power
...
The majority of cosmic shear analyses have used \mk{the predicted} lensing
power spectrum approximated on a flat-sky, neglecting the sky curvature. This
is a valid approach for past and current survey areas with an extent less than
10 degrees. \mk{To account for the sky curvature of the observed data}, the
shear correlation functions \mk{from observed galaxy ellipticities is now
routinely} computed using spherical coordinates, \mk{since projecting to a
Cartesian plane has been shown to cause significant biases of the two-point
funtion on large scales \citep{FSHK08}}. Here we examine the effect of sky
curvature on the theoretical \mk{models of the power spectrum, and the effect
on cosmological parameter inference (see Sect.~\ref{sec:cfhtlens}).}
...
\end{equation}
%
The spin pre-factor $\sqrt{(\ell-1) \ell (\ell+1) (\ell+2)}$ is replaced by
$\ell^2$ in flat co-ordinates, \mk{an approximation that holds for large $\ell$.}
 \ch{@MK - say why this is so.  Is it just that
flat by definition means large $\ell$?} \mk{I think it's a large-ell approximation,
but I don't know whether this is an assumption to derive this approximation, or whether it follows from that.
I'll have to check this.}\MA{I am pretty sure that flat means large $\ell$, as the sky is flatter for smaller angular scales.}
We find for the flat-sky shear power
spectrum
%
\begin{align}
  P^\gamma_{ij}(\ell) 
...
all-sky case. \mk{We apply their general derivation to the case of weak lensing,
...
\end{equation}
%
in equation~(\ref{eq:C_ell_full}), \MA {where ${\rm J}_\ell$ is the Bessel function of the first kind
and order $\ell$}. Next, \cite{2008PhRvD..78l3506L} solve integrals of
the form
%
\begin{equation}
...
\end{equation}
%
by performing a Taylor expansion of \mk{an arbitrary differentialble} function
...
\end{equation}
%
\mk{TODO: expand more on this. Explain why there is a non-zero correlation across epochs.}
...
k^{-1-n} f^{(n)}(k, \chi)$ for a given $k$, \mk{where the derivatives of $f$
are with respect to the second argument $\chi$.} \mk{This series expansion, the
...
$g$ is varying more slowly than the oscillating Bessel function [Check this].
As we will see below, $f$ is a slowly varying function of the comoving
...
\end{equation}
%
\mk{In the tomographic case, the index $i$ is added to $q$ and $f$.}
%
...
\begin{align}
  C^\gamma_{{\rm L2}, ij}(\ell) = & - \frac 1 {\nu^2(\ell)} \, \frac{\ellbar^2(\ell, 2)}{\nu^4(\ell)} \, \pref^2
    \int {\rm d} \chi \, \chi^{2} a^{-2} P^{1/2}_{\rm m}\left(\frac{\nu(\ell)}\chi; \chi\right)
    %\nonumber \\
    %& \times
...
\subsubsection{\mk{Facoring out the growth factor}}
\label{sec:factor_D}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
\mk{The function $\tilde P_{\rm m}$ is in general not time-independent, except
...
\mk{With the definition (\ref{eq:P_k_chi_sep_s}), we factor out the function
...
\end{equation}
%
The second-order Limber term of the shear power spectrum can then be approximated as
%
\begin{align}
  C^\gamma_{{\rm L2}, ij}(\ell) = & - \frac 1 {\nu^2(\ell)} \, \frac{\ellbar^2(\ell, 2)}{\nu^4(\ell)} \, \pref^2
...
$10^{-4}$ of its value of unity at $1$\,Mpc$/h$.
...
spherical case (\ref{eq:C_ell_limber1}, \ref{eq:C_ell_limber2_dr_s}), by
...
\begin{align}
  ...
  \\
    P^\gamma_{{\rm L2}, ij}(\ell) = & - \frac 1 {\nu^2(\ell)} \, p(\ell) \, \pref^2
    \int {\rm d} \chi \, \chi^{2} a^{-2} P_{\rm m}\left(\frac{\nu(\ell)}\chi; \chi\right)
    %\nonumber \\
    %& \times
    \frac{a(\chi)}{2 D_+(\chi)} \chi^{3/2} \left[ q^{}_i f^{\prime\prime}_{{\rm s}j} + f^{\prime\prime}_{{\rm s}i} q^{}_j  
      + \frac{\chi}{3} \left( q^{}_i f^{\prime\prime\prime}_{{\rm s}j} + f^{\prime\prime\prime}_{{\rm s}i} q^{}_j
      \right)
    \right](\chi)
...
\end{align}
%
Further approximations are \mk{can} made for the pre-factor $p(\ell) = \ell^4/\nu^4(\ell)$ and $\nu(\ell)$:
%
\begin{enumerate}
  \item $p(\ell) = 1$, this corresponds to $\nu(\ell) = \ell$, which is the
    ...
    Note that we do not discuss the non-extended second-order Limber approximation here.
  %
  ...
  %
  \item $p(\ell) = 1$, \mk{but} keeping as argument of the power spectrum $\nu(\ell) = \ell + 1/2$. This is
    a \emph{hybrid} between
    standard and extended Limber approximation \Label{ExtL1FlHyb,ExtL2FlHyb}, and this case of first order
    was used in \cite{KiDS-450,joudaki/etal:2016}. As is shown
    later, this is a better approximation to the full projection than case (ii). \mk{In (\ref{eq:P_ell_limber2})
...
approximation is a factor of \mk{15 times} faster than the calculation of the
full spherical solution \mk{(averaged over the first $18$ $\ell$-modes).} We
note that the \mk{sub-$0.1\%$}-noise seen in the right panel of
...
\mk{Compared to the statistical power of current and future surveys, all
approximations discussed here are subdominant to the cosmic variance, $\Delta
C(\ell) / C(\ell) = [f_{\rm sky} (2 \ell + 1)]^{-1/2}$
\citep{1992ApJ...388..272K}, where $f_{\rm sky}$ is the faction of sky observed
...
\begin{figure}
  ...
  \caption{\label{fig:Cl_cases}%
        ...
        relative to the full spherical projection of the shear power spectrum. \mk{The two light grey curves on the
...
\begin{table}
\begin{centering}
  \caption{\label{tab:cases}The shear power spectrum approximations studied in this paper. 'ID' is the \mk{label} used in the \mk{text and} figures.
  ...
  $1^{\rm st}$-order standard Limber, flat sky & L1Fl & (\ref{eq:P_ell_limber1}) + (i)
    & $1$ & $\ell$ & Used for pre-2014 CFHTLenS \\ \hline
  %
  ...
  $1^{\rm st}$-order extended Limber, hybrid, flat sky & ExtL1FlHyb & (\ref{eq:P_ell_limber1}) + (iii)
    & $1$ & $\ell + \frac 1 2$ & Used for post-2014 CFHTLenS, KiDS \\ \hline
  %
...
On a sphere, correlation functions \mk{formally} cannot be related to the power
spectrum by the Hankel transform in equation~(\ref{eqn:xiGG}), \mk{and should
be replaced by a Legendre transform}. The spherical power spectrum is formally
not defined for non-integer $\ell$ \citep[see][for alternative spherical-sky
formulae for the two-point correlation function]{2005PhRvD..72b3516C}, \mk{as
functions defined on the sphere are necessarily periodic}. \sout{In particular,
the spherical Bessel functions necessary to compute the full projection in
equation~(\ref{eq:C_ell_full}) is defined only for integer order $\ell$.}
%\mk{I think this is actually not true, since J$_\nu$ can be computed
%for fractional $\nu$, and then one could use the identity (\ref{eq:jJ}). Or is
%it?}.
...
%In the Limber approximations
%the Bessel functions \mk{of order $\nu$ are expanded into} polynomials \mk{in
%$\nu$}, allowing for arbitrary wave modes $\ell$.
...
thus use the spherical power spectrum in the Limber approximations for the
Hankel transformation to compute the two-point correlation functions. \mk{This
has the advantage of very fast numerical implementations of the Hankel
tranforms using FFTs \citep{fftlog}, which can be used in
...
projection with ExtL2Sph for $\ell > 200$ to save computation time. We see no
qualitative differences between the three cases.} \mk{[Note: The differences in
the actual figure probably come from the difference n $\ell_{\rm max}$, to be
updated.]}

%We therefore do not present predictions for the
%two-point shear correlation function using the full spherical projection,
%instead choosing the spherical second-order extended Limber approximation
%(ExtL2Sph) as our reference in this case. \mk{Todo: Include Ludo's spherical
%calcuations of $\xi_\pm$.}

Figure.~\ref{fig:xi_pm} presents the Hankel-transform two-point correlation
functions $\xi_+$ (left) and $\xi_-$ (right) using the different cases for the
shear power spectrum listed in Table~\ref{tab:cases}. \mk{We use `ExtL2Sph' as reference case.}
The adopted CFHTLenS
redshift distribution and fiducial cosmological model are the same as in
...
\begin{figure}
  ...
  \caption{The shear correlation function component $\xi_+$ for the ExtL2Sph
projection using the Hankel transform (green line), compared to the spherical
Legendre transform of the same projection (blue squares), and the FullSph
projection (red circles).}
...
\begin{equation}
  \psi_{\ell m} = \frac 2 {c^2} \int {\rm d} \Omega {\rm Y}^\ast_{\ell m}(\theta, \varphi)
    \int_0^{\chi_{\rm lim}} \frac{{\rm d}\chi}{\chi} q(\chi) \int \frac{{\rm d}^3 k}{(2\pi)^3} \hat \Phi(\vec k; \chi) {\rm e}^{-{\rm i} \vec k \cdot \vec r}.
\end{equation}
%
...
\end{equation}
%
\mk{To obtain the potential power spectrum,} we take the absolute square of the
...
z_{\rm p})$ \mk{is unity if $z_{\rm p}$ is in the redshift bin denoted by $z$,
...
calculated using $g(a)$ instead of $D(a)$. \mk{Check this further.}
