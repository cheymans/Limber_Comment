...
above a coherence scale $|\chi - \chi^\prime| \gsim L_{\rm coh}$ which is much
...
CFHTLenS redshift range.} The fit is performed between $\chi_{\rm min} = 0.001
...
\begin{enumerate}
  ...
  \item $p(\ell) = 1$, but keeping as argument of the power spectrum $\nu(\ell) = \ell + 1/2$. This is
    ...
    standard and extended Limber approximation \Label{ExtL1FlHyb,ExtL2FlHyb}, and the first-order case 
    was used in \cite{KiDS-450,joudaki/etal:2016,joudaki/etal:2017}. As is shown
    below, this is a better approximation to the full projection than case (ii). In (\ref{eq:P_ell_limber2})
...
%
The Hankel transform (\ref{eqn:xiGG}) is accurate at the $1$ percent level.
%
