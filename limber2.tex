...
second-order Limber approximation will provide sufficient accuracy. \mk{The
cosmic-shear power spectrum} is in agreement with the full projection at the
sub-percent level for $\ell > 3$, and the corresponding errors an order of
magnitude below cosmic variance for all $\ell$. \mk{The shear two-point
correlation function is accurate well below up to a few hundred arc minutes.
Using the fast Hankel transformation, formally defined only in flat
coordinates, to compute the two-point function from the power spectrum results
...
redshift distribution $n_z(z) {\rm d}z = n(\chi) {\rm d} \chi$, with limit the
...
cross-spectrum \cite{2016arXiv161200770K} will be further evaluated and
simplified in the context of the Limber approximation. The oscillating Bessel
functions  in (\ref{eq:C_ell_phi_Pm}) ensure that only relatively close epochs
...
above a coherence scale $|\chi - \chi^\prime| \le L_{\rm coh}$ which is much
smaller than the horizon scale. }
...
The majority of cosmic shear analyses have used the predicted lensing
...
10 degrees. To account for the sky curvature of the observed data, the
shear correlation functions from observed galaxy ellipticities is now
routinely computed using spherical coordinates, since projecting to a
...
curvature on the theoretical models of the power spectrum, and the effect
...
\end{equation}
%
The spin pre-factor $\ellbar(\ell, 2) = \sqrt{(\ell-1) \ell (\ell+1) (\ell+2)}$ is replaced by
$\ell^2$ in flat co-ordinates, an approximation that holds for large $\ell$.
...
all-sky case. We apply ther general derivation to the case of weak lensing,
...
k; \chi) = D_+(\chi) \hat \delta_0(\vec k)$ into (\ref{eq:p_m}), whhere
$\delta_0$ is the present-day linearly extrapolated density contrast, and $D_+$
the linear growth factor with $D_+(0) = 1$. This is a very good approximation
...
\subsubsection{Facoring out the growth factor}
\label{sec:factor_D}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{align}
%
Further approximations are can made for the pre-factor $p(\ell) = \ell^4/\nu^4(\ell)$ and $\nu(\ell)$:
%
\begin{enumerate}
  ...
  \item $p(\ell) = 1$, but keeping as argument of the power spectrum $\nu(\ell) = \ell + 1/2$. This is
    ...
    later, this is a better approximation to the full projection than case (ii). In (\ref{eq:P_ell_limber2})
...
note that the sub-$0.1\%$-noise seen in the right panel of
...
be replaced by a Legendre transform,
%
\mk{
\begin{equation}
...
thus use the spherical power spectrum \sout{in the Limber approximations} in
the Hankel transformation (\ref{eqn:xiGG}) to compute the two-point correlation
functions. This has the advantage of very fast numerical implementations of
the Hankel tranforms using FFTs \citep{fftlog}, which can be used in
...
projection with ExtL2Sph for $\ell > 200$ to save computation time.
%
2he Hankel transform (\ref{eqn:xiGG}) is accurate at the $1$ percent level.
%
...
for all but the very large scales.}

\mk{The green curve in Fig.~\ref{fig:xi_p_legendre} presents the limit of
precision we can achieve with the current fast Hankel transform implementation
for the correlation function. The blue points shows the limit of the
second-order Limber approximation on the correlation function.}

Figure~\ref{fig:xi_pm} shows the Hankel-transform two-point correlation
functions $\xi_+$ (left) and $\xi_-$ (right) using the different cases for the
shear power spectrum listed in Table~\ref{tab:cases}. \mk{We use `ExtL2Sph' as
reference case.} The adopted CFHTLenS redshift distribution and fiducial
cosmological model are the same as in Figure~\ref{fig:Cl_cases}. As is clear by
the red dotted curve, using the Planck cosmology \citep{2015arXiv150201589P}
induces a significant change in the amplitude of the shear correlation function
in comparison to the different projection methods.
...
for the ExtL2Sph projection using the Hankel transform (green line), compared
to the spherical Legendre transform of the same projection (blue squares), and
the FullSph projection (red circles). \emph{Left panel:} The relative
difference with respect to the full spherical case (FullSph with Legendre
transformation) of ExtL2Sph with Hankel transform (green line), and ExtL2Sph
with Legendre transform (blue squares).}
...
calculated using $g(a)$ instead of $D(a)$. \mk{TODO: Check this further.}
